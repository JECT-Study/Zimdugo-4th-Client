interface ResolveMapAttachFocusOptions {
  ...
  /** 로더가 가져온 상세. 좌표가 여기 있어야 상세로 맞출 수 있다. */
  hasDetail: boolean;
  /** 지도가 없어 미뤄 둔 핀이 있는지. */
 ...
 *
 * 판단이 셋 엮여 있어 따로 뺐다. 상세가 핀보다 앞선다는 것, 같은 보관함은 한 번만
 * 맞춘다는 것, 그리고 좌표가 없으면 상세가 성립하지 않는다는 것이다.
 *
 * **같은 보관함을 한 번만 맞추는 이유**: 테마를 바꾸면 지도를 다시 만든다. 그때마다
 * 다시 맞추면 사용자가 옮겨 둔 위치를 덮어쓴다.
 *
 * 상세의 좌표가 없으면 `"pin"` 으로 내려간다. 상세를 열고는 있지만 어디인지 모르는
 * 상태라, 미뤄 둔 핀이 있으면 그쪽이 더 나은 답이다.
 */
