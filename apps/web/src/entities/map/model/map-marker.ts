...
const USE_PLACE_BADGE_MARKER = true;
...
const createDefaultLockerMapPinSvg = (
  fill: string = PLACE_BADGE_FILL,
): string => `
  <svg width="100%" height="100%" viewBox="0 0 90 90" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style="display: block;">
    <g clip-path="url(#default-map-pin-clip)">
      <g filter="url(#default-map-pin-shadow)">
    ...
    <defs>
      ...
      <clipPath id="default-map-pin-clip">
...
const getMarkerFill = (pin: LockerPinItemResponse): string =>
  pin.markerStatus === "inactive" ? INACTIVE_MARKER_FILL : PLACE_BADGE_FILL;

const createPlaceClusterMapPinSvg = (
  badgeLabel: string,
  fill: string = PLACE_BADGE_FILL,
): string => {
  ...
  if (!isPlace || !USE_PLACE_BADGE_MARKER) {
    ...
    const icon = isInactive
      ...
      : isFavorite
        ...
        : createDefaultLockerMapPinSvg(getMarkerFill(pin));
    const variant = isInactive ? "inactive" : isFavorite ? "save" : "default";

    return `<div data-type="${pin.pinType}" data-map-pin-variant="${variant}" style="position: relative; display: block; width: 100%; height: 100%;">
      ${icon}
    </div>`;
  }

  return `<div data-type="${pin.pinType}" data-map-pin-variant="${pin.markerStatus === "inactive" ? "inactive" : "cluster"}" style="position: relative; display: block; width: 100%; height: 100%;">
...
const createMarkerIconOptions = (
  ...
  isFavoriteAnimationActive = false,
): naver.maps.HtmlIcon => {
  const hasSpread =
  ...
  }${isFavoriteAnimationActive ? ":favorite-added" : ""}${
    hasOffset ? `:offset:${offsetX}:${offsetY}` : ""
  }`;
  ...
  const offsetAttributes = hasOffset
    ...
    : "";
  const favoriteAnimationClass = isFavoriteAnimationActive
    ? "favorite-added"
    : "";
...
const createLockerMarker = ({
  ...
  isFavoriteAnimationActive?: boolean;
  zIndex: number;
}) => {
...
export const syncLockerMarkers = ({
  ...
  for (const pin of lockers) {
    ...
    if (existingEntry) {
      ...
      const isFavorite = pin.pinType === "LOCKER" && pin.isFavorite === true;
      let isFavoriteAnimationActive = false;
      if (isFavorite && existingEntry.hasFavoriteBefore === false) {
        isFavoriteAnimationActive = true;
      ...
      const favoriteAnimationSignature = isFavoriteAnimationActive
        ? ":favorite-added"
        : "";
      ...
      existingEntry.hasFavoriteBefore = isFavorite;
    ...
    const entry: LockerMarkerEntry = {
      ...
      hasFavoriteBefore: pin.pinType === "LOCKER" && pin.isFavorite === true,
